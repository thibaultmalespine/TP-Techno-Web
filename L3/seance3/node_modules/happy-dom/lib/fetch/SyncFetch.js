...
class SyncFetch {
     ...
     */
    send() {
        FetchRequestReferrerUtility.prepareRequest(__classPrivateFieldGet(this, _SyncFetch_window, "f").location, this.request);
    ...
    compliesWithCrossOriginPolicy() {
        if (this.disableCrossOriginPolicy ||
            !FetchCORSUtility.isCORS(__classPrivateFieldGet(this, _SyncFetch_window, "f").location, this.request[PropertySymbol.url])) {
            return true;
        }
            ...
            case 'follow':
                ...
                if (this.request.credentials === 'omit' ||
                    (this.request.credentials === 'same-origin' &&
                        FetchCORSUtility.isCORS(__classPrivateFieldGet(this, _SyncFetch_window, "f").location, locationURL))) {
                ...
                return fetch.send();
            default:
                throw new DOMException(`Redirect option '${this.request.redirect}' is not a valid value of RequestRedirect`);
        }
    }
}
